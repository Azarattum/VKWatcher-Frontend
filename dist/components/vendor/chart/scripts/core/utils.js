    ...
    get() {
        ...
        if (typeof this.startProperty == "number") {
            ...
            for (let i = 0; i < this.startProperty.length; i++) {
                animated.push(this.interpolate(this.startProperty[i], this.endProperty[i], progress));
            }
            return animated;
        }
        else if (this.startProperty instanceof Color) {
            const progress = (timePast / this.duration);
            return new Color(this.interpolate(this.startProperty.r, this.endProperty.r, progress), this.interpolate(this.startProperty.g, this.endProperty.g, progress), this.interpolate(this.startProperty.b, this.endProperty.b, progress), this.interpolate(this.startProperty.a, this.endProperty.a, progress));
        }
        else if (this.startProperty instanceof Point) {
            const progress = (timePast / this.duration);
            return new Point(this.interpolate(this.startProperty.x, this.endProperty.x, progress), this.interpolate(this.startProperty.y, this.endProperty.y, progress));
        }
    }
