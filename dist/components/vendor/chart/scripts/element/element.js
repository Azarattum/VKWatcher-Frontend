...
export default class ChartElement {
    ...
    set chart(chart) {
        ...
        this.drawer = new ChartDrawer(this.chartData, this.elements.chart, this.shaders, this.elements.layout);
        this.previewer = new ChartDrawer(this.chartData, this.elements.preview, this.shaders);
        this.controller = new ChartController(this.elements.select, this.elements.draggerLeft, this.elements.draggerRight, this.elements.layout);
        ...
        this.controller.onupdate = (start, end) => {
            ...
            const text = document.createTextNode(startDate.getDate() + " " +
        ...
        this.drawer.onrecalculated = () => {
            if (this.drawer.selection.value == null)
                return;
            //Update the date
            const index = this.drawer.selection.index;
            let date = new Date(this.drawer.chart.xAxis[index]).toString().split(" ");
            ...
            for (let i = 0; i < values.length; i++) {
                ...
                if (this.chartData.percentage) {
                    ...
                    values[i].children[0].innerHTML = Math.round(this.drawer.chart.graphs[i].points[index].y / sum * 100) + "%";
                }
    ...
    set style(styles) {
        ...
        this._initializeStyle();
        if (this.drawer) {
            this.drawer.update(new Color(this.styles.background), new Color(this.styles.text), styles.font, 5);
        }
        if (this.previewer) {
            this.previewer.update(new Color(this.styles.background), new Color(this.styles.text), styles.font, 2);
        }
    }
    set title(title) {
