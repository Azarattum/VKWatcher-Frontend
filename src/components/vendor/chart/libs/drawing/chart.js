...
import LayoutDrawer from "./layout.js";
import LineGraphDrawer from "./graphLine.js";
import BarGraphDrawer from "./graphBar.js";
import AreaGraphDrawer from "./graphArea.js";
...
export default class ChartDrawer {
	...
	constructor(chart, canvas, shadersPack, layout = null) {
		...
		for (const graph of chart.graphs) {
			if (graph.type == "line") {
				this.graphDrawers.push(
					new LineGraphDrawer(this, graph, this.gl, shadersPack.line)
				);
			} else if (graph.type == "bar") {
				this.graphDrawers.push(
					new BarGraphDrawer(this, graph, this.gl, shadersPack.bar)
				);
			} else if (graph.type == "area") {
				this.graphDrawers.push(
					new AreaGraphDrawer(this, graph, this.gl, shadersPack.area)
				);
			}
		}
	...
	set select(value) {
		const selection =
			value == null
				? null
				: (this.area.start +
						value / (1 / (this.area.end - this.area.start))) *
						2 -
				  1;
	...
	_calculate() {
		const amplitude =
			this.chart.type != "bar"
				? 0
				: (this.graphDrawers[0].path.points[1].x -
						this.graphDrawers[0].path.points[0].x) /
				  2;
		//Estimate index
		...
		while (true) {
			...
			if (this.layout && this.selection.input != null) {
				let distance = Math.abs(
					this.selection.input - vertex.x - amplitude
				);
		...
		const moveY = zoomY - 1;
		const projection = [zoomX, 0, 0, 0, zoomY, 0, moveX, moveY, 1];
		...
		if (selectionIndex != -1) {
			for (const drawer of this.graphDrawers) {
				this.selection.points.push(
					drawer.graph.vertices[selectionIndex].y * zoomY + moveY
				);
			}
		}
	...
	draw() {
		if (this.redraw) {
			...
			if (
				!this.graphDrawers.some(x => x.animating) &&
