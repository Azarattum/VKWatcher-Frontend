...
import Day from "../data/day.class.js";
...
export default class Drawer {
    ...
    private _styles: {
        ...
        time: {
            margin: number, left: number, fontSize: number,
     ...
     */
    constructor(canvas: HTMLCanvasElement, user: User | null = null) {
    ...
    private drawData(ctx: CanvasRenderingContext2D, days: Day[]): void {
        ...
        for (let i = 0; i < days.length; i++) {
            ...
            if (inSleep != null) {
                ctx.globalAlpha = 0.3;
                this.shadeRect(ctx, x, inSleep, width - margin,
                    (this.viewport.height - height) - inSleep, 0.15);
                ctx.globalAlpha = 1;
                inSleep = 0;
            }
        }
    ...
    private shadeRect(ctx: CanvasRenderingContext2D, x = 0, y = 0, width = 0, height = 0, density = 0.1) {
