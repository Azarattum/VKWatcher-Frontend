...
async function initialize() {
    ...
    try {
        ...
        Utils.log(`Data initialization exception:\n\t` +
    ...
    try {
        ...
        Utils.log(`Interface initialization exception:\n\t$` +
            `${exception.stack.replace(/\n/g, "\n\t")}`,
            LogType.ERROR);
        exceptions++;
    }
    ...
    try {
        ...
        Utils.log(`Hash initialization exception:\n\t` +
            `${exception.stack.replace(/\n/g, "\n\t")}`,
            LogType.ERROR);
        exceptions++;
    }
    ...
    try {
        ...
        Utils.log(`Overview initialization exception:\n\t` +
            `${exception.stack.replace(/\n/g, "\n\t")}`,
            LogType.ERROR);
        exceptions++;
    }
    ...
    if (exceptions) {
        Utils.log(`Initialization completed with ${exceptions} exceptions!`,
            LogType.WARNING);
    } else {
...
async function initializeInterface() {
    ...
    Interface.addEventListener("zoomed", (factor: number) => {
        ...
        (document.getElementsByClassName("page")[0] as HTMLElement)
            .style.setProperty("--vertical-zoom", factor.toString());
    });
